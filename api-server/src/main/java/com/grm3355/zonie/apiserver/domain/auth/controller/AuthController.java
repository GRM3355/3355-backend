...
public class AuthController {
	...
	@ApiResponses({
		@ApiResponse(responseCode = "200", description = "로그인 성공", content = @Content(mediaType = "application/json", schema = @Schema(implementation = CustomApiResponse.class),
	...
	public ResponseEntity<?> login(@AuthenticationPrincipal UserDetailsImpl user,
		...
		if (user != null && user.getUsername() != null && !user.getUsername().isBlank()) {
			...
			return CustomApiResponse.created(response).toResponseEntity();
		}
